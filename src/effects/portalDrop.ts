...
export async function play(ctx: EffectContext): Promise<void> {
	...
	await machine.openClamps(480);
	haptics.vibrate(40);

	// iris open + camera pushes in slightly
	scene.fxLight.color.set(0xeaf6ff);
	tween(1200, 'inOutQuad', (v) => (scene.fxLight.intensity = v * 3.5));
	scene.fxLight.position.set(0, -0.3, 0.6);
	const rigZ = scene.rig.position.z;
	tween(1800, 'inOutCubic', (v) => (scene.rig.position.z = rigZ - v * 0.55));
	await machine.openIris(0.72, 1200); // just clear of the portal edge — no big gap

	// the button drops away, tumbling
	haptics.vibrate(60);
	const drop = tween(1500, 'inQuart', (v) => {
	...
	tween(1300, 'inOutCubic', (v) => (scene.rig.position.z = rigZ - 0.55 * (1 - v)));
	tween(900, 'outQuad', (v) => (scene.fxLight.intensity = 3.5 * (1 - v)));
	await machine.closeIris(1050);
	await machine.closeClamps(450);
	await flashPulse(machine, 0.7, 100, 560, 0xeaf6ff);
}
