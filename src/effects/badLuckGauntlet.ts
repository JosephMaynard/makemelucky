...
//
// Sound: /soundfx/still-lucky-tonight.mp3 (18.4s, 128bpm electro-swing).
// Lyrics 0-15s: "Black cat, ladder, umbrella too, / Bad luck, I ain't scared
// of you! / Superstition won't bring me down, / 'Cause I'm still lucky
// tonight!" + reprise "I'm still lucky tonight!". The choreography is pinned
// to the song with syncTo(): the turret blast lands on "Bad luck!" (~3.75s)
// and the shield detonation + DONG land on the final "tonight!" (15s).
// Only the button and the force field bop to the beat — the machine itself
// stays deadpan; that's the joke.
...
export const sound = "stillLuckyTonight";
export const duration = 18000;

const BEAT = 60 / 128; // 0.469s
...
function buildCat(glow: THREE.Texture): {
  ...
  const torsoGeo = G(
    new THREE.LatheGeometry(bodyProfile, BODY_SEGS, -Math.PI / 2),
  );
  ...
  for (let i = 0; i < tPos.count; i++) {
    ...
    const cy =
      0.32 +
      0.055 * u * u +
      (0.132 - Math.hypot(rv, rw)) * 0.7 * Math.sin(Math.PI * u);
    tPos.setXYZ(
      i,
      -0.3 + u * 0.62,
      cy + rv * (1.16 - 0.12 * u),
  ...
  // out of it
  const scruff = new THREE.Mesh(
    G(new THREE.SphereGeometry(0.072, 20, 16)),
    fur,
  );
  ...
  const neck = new THREE.Mesh(
    G(new THREE.CapsuleGeometry(0.071, 0.085, 6, 18)),
    fur,
  );
  ...
  head.add(skull);
  const cheeks = new THREE.Mesh(
    G(new THREE.SphereGeometry(0.062, 20, 16)),
    fur,
  );
  ...
  const muzzle = new THREE.Mesh(
    G(new THREE.SphereGeometry(0.048, 18, 14)),
    fur,
  );
  ...
  const nose = new THREE.Mesh(
    G(new THREE.SphereGeometry(0.014, 10, 8)),
    noseMat,
  );
  ...
  const hindGeo = limb(
    ...
    (t) =>
      0.04 * Math.sin(Math.PI * Math.min(1, t / 0.7)) - 0.028 * step(0.7, 1, t),
    0.345,
  );
  ...
  const tailGeo = G(
    new THREE.TubeGeometry(path, TUBE_SEGS, 1, RING - 1, false),
  );
  ...
  const setTailPuff = (p: number): void => {
    for (let i = 0; i < nT; i++) {
      ...
      const r =
        (0.038 - 0.013 * u - 0.01 * Math.pow(u, 3.2)) *
        cap *
        (1 + (p - 1) * brush);
      tPosT.setXYZ(
        i,
...
function buildUmbrella(): { umbrella: THREE.Group } {
  ...
  const canopy = new THREE.Mesh(
    new THREE.ConeGeometry(1.05, 0.52, 8, 1, true),
    cloth,
  );
  ...
  const shaft = new THREE.Mesh(
    new THREE.CylinderGeometry(0.026, 0.026, 1.5, 8),
    ribMat,
  );
  shaft.position.y = -0.4;
  umbrella.add(shaft);
  const hook = new THREE.Mesh(
    new THREE.TorusGeometry(0.11, 0.026, 8, 16, Math.PI * 1.2),
    ribMat,
  );
...
export async function play(ctx: EffectContext): Promise<void> {
  ...
  // the song is the master clock: waits are absolute marks on the track, so
  // tween jitter can't drift the choreography off the lyrics
  const t0 = performance.now();
  const syncTo = (ms: number): Promise<void> => {
    const remaining = ms - (performance.now() - t0);
    return remaining > 16 ? delay(remaining) : Promise.resolve();
  };
  ...
  // ---- the beat sim: the button and the shield bop along; nothing else
  // does. A half-tempo nod (64bpm) between two poses: a 3% squash at the
  // bottom and a 4% stretch at the top. Each move arrives early in its beat
  // and settles with a visible-but-damped wobble — life, not jelly.
  const btnScale0 = machine.buttonGroup.scale.clone();
  const btnZ0 = machine.buttonGroup.position.z;
  const btnRot0 = machine.buttonGroup.rotation.z;
  let shieldT = 0;
  let songT = 0;
  let groove = 0; // how hard we're bopping, eased in and out
  let bopOn = true; // false once the verdict owns the button + uniforms
  let hitSlot = 0;
  let chargeVis = 0; // tweened charge level; the sim layers the beat on top
  const stopSim = scene.addUpdatable((dt) => {
    shieldT += dt;
    songT += dt;
    shieldMat.uniforms.uTime.value = shieldT;
    if (!bopOn) return;
    // where are we in this beat's move, with a whisper of settle at the end
    const p = (songT % BEAT) / BEAT;
    const eased = 1 - Math.pow(1 - Math.min(1, p / 0.35), 3);
    const settle =
      p > 0.35
        ? Math.sin((p - 0.35) * Math.PI * 4) * Math.exp(-(p - 0.35) * 5) * 0.16
        : 0;
    const step = eased + settle;
    // beats alternate: even beats travel to the DOWN pose, odd back UP
    const down = Math.floor(songT / BEAT) % 2 === 0;
    const dRaw = down ? step : 1 - step;
    const d = dRaw * groove;
    // the pose axis: stretched 1.04 at the top of the nod, 0.97 at the bottom
    const sy = 1 + (1.04 - dRaw * 0.07 - 1) * groove;
    machine.buttonGroup.scale.set(
      btnScale0.x * (1 + (1 - sy) * 0.8),
      btnScale0.y * sy,
      btnScale0.z * (1 + (1 - sy) * 0.8),
    );
    machine.buttonGroup.position.z = btnZ0 + d * 0.04;
    machine.buttonGroup.rotation.z =
      btnRot0 + Math.sin((songT * Math.PI) / (BEAT * 4)) * 0.03 * groove;
    // the force field breathes with the nod
    shieldMat.uniforms.uBase.value = chargeVis + d * 0.1;
    shield.scale.setScalar(1 + d * 0.025);
  });
  tween(1000, "inOutQuad", (v) => (groove = v));
  ...
  tween(600, "outQuad", (v) => (chargeVis = v * shieldCharge));
  const chargeShield = (to: number) => {
    const from = shieldCharge;
    shieldCharge = to;
    tween(500, "outQuad", (v) => (chargeVis = from + (to - from) * v));
  };
  ...
  // "Black cat, ladder, umbrella too..." — the roll call plays over the setup
  await syncTo(700);
  ...
  await syncTo(3700);
  // FIRE — right on "Bad luck!". Twin lucky lasers; the ladder becomes a
  // firework of splinters.
  ...
  // "...I ain't scared of you!" — enter the cat, unbothered
  await syncTo(4900);
  const { cat, legs, tail, ears, head, setTailPuff } = buildCat(
    sprites.softDot,
  );
  const startX = 3.4;
  ...
  const stopCat = scene.addUpdatable((dt, t) => {
    ...
    cat.position.y = Math.max(
      cat.position.y,
      -1.02 + Math.abs(Math.sin(walk)) * 0.02,
    );
  });
  // crosses your path, right to left
  await tween(1800, "linear", (v) => {
  ...
  await tween(
    350,
    "inOutQuad",
    (v) => (head.rotation.z = 0.35 - Math.sin(v * Math.PI) * 0.12),
  ); // sniff sniff
  // "Superstition won't bring me down" — the defence system fires a WARNING
  // SHOT into the floor. Nobody shoots the cat. The cat is merely advised.
  await syncTo(7550);
  lightning.strike(
  ...
  await syncTo(12200); // "'Cause I'm still lucky tonight!" — let the absurd
  // little scene be ENJOYED right through the line.
  ...
  shockwave(
    scene.scene,
    new THREE.Vector3(brolly.position.x, brolly.position.y - 0.2, 1.0),
    {
      color: 0xffd27a,
      maxScale: 2.2,
      duration: 480,
  ...
  rain.stop();
  // ================= the verdict =================
  // The reprise winds up — the band hands the stage to the shield: the
  // button eases out of its bop and the verdict owns the uniforms now.
  await syncTo(14300);
  bopOn = false;
  {
    const bs = machine.buttonGroup.scale.clone();
    const bz = machine.buttonGroup.position.z;
    const br = machine.buttonGroup.rotation.z;
    tween(260, "outQuad", (v) => {
      machine.buttonGroup.scale.lerpVectors(bs, btnScale0, v);
      machine.buttonGroup.position.z = bz + (btnZ0 - bz) * v;
      machine.buttonGroup.rotation.z = br + (btnRot0 - br) * v;
    });
  }
  ...
  // ...and DETONATES as pure lucky energy on the final "tonight!". For good
  // luck. Obviously.
  await syncTo(15000);
  ...
  // ---- teardown
  stopSim();
  machine.buttonGroup.scale.copy(btnScale0);
  machine.buttonGroup.position.z = btnZ0;
  machine.buttonGroup.rotation.z = btnRot0;
