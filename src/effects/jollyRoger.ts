...
function buildSkull(glowTex: THREE.Texture) {
	...
	head.add(nose);
	// the maw: a dark interior behind both rows. Teeth only read as teeth
	// against a dark void — without it they merge into one pale brick.
	const maw = new THREE.Mesh(new THREE.SphereGeometry(0.115, 16, 12), darkMat);
	maw.scale.set(1.05, 0.58, 0.62);
	maw.position.set(0, -0.2, 0.085);
	head.add(maw);

	// upper teeth — seven, TOUCHING (a gap between every tooth reads as a comb,
	// not a skull), one gold, obviously. The bone arc behind them means any
	// sliver that does show is bone rather than black.
	const upperGum = new THREE.Mesh(
		new THREE.TorusGeometry(0.128, 0.026, 8, 26, Math.PI * 1.05),
		boneMat
	);
	...
	for (let i = 0; i < 7; i++) {
		const a = (i / 6 - 0.5) * 1.5;
		const w = i === 3 ? 0.045 : 0.042;
		const tooth = new THREE.Mesh(new THREE.BoxGeometry(w, 0.05, 0.03), i === 2 ? goldMat : boneMat);
		tooth.position.set(Math.sin(a) * 0.128, -0.185, 0.055 + Math.cos(a) * 0.112);
		tooth.rotation.y = a;
		head.add(tooth);
	}

	// jaw, pivoted at the back so a POSITIVE rotation.x drops the chin — the
	// lyric animation drives this and must not change.
	const jaw = new THREE.Group();
	jaw.position.set(0, -0.19, 0.0);
	// the mandible is ONE curved bone. Built from separate blocks it had a
	// visible gap at every joint, which is what spoiled it.
	const mandible = new THREE.Mesh(
		new THREE.TorusGeometry(0.128, 0.03, 8, 28, Math.PI * 1.08),
		boneMat
	);
	...
	jaw.add(chin);
	for (const side of [-1, 1]) {
		const ramus = new THREE.Mesh(new THREE.BoxGeometry(0.046, 0.115, 0.05), boneMat);
		ramus.position.set(side * 0.126, 0.025, -0.008);
		ramus.rotation.x = -0.44;
		jaw.add(ramus);
	}
	for (let i = 0; i < 6; i++) {
		const a = (i / 5 - 0.5) * 1.3;
		const tooth = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.04, 0.026), boneMat);
		tooth.position.set(Math.sin(a) * 0.122, -0.016, 0.06 + Math.cos(a) * 0.112);
		tooth.rotation.y = a;
		jaw.add(tooth);
	}
	head.add(jaw);
