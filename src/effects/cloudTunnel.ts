...
let tunnel: ReturnType<typeof buildTunnel> | null = null;

function buildTunnel(ctx: EffectContext) {
	const { sprites } = ctx;
...
export async function play(ctx: EffectContext): Promise<void> {
	...
	const fogC0 = (scene.scene.fog as THREE.Fog).color.clone();
	const bgC = scene.scene.background as THREE.Color;
	...
	await tween(1000, 'inCubic', (v) => {
		...
		(scene.scene.fog as THREE.Fog).far = 52 - v * 46;
		(scene.scene.fog as THREE.Fog).near = 7 - v * 6.2;
		(scene.scene.fog as THREE.Fog).color.lerpColors(fogC0, white, v);
