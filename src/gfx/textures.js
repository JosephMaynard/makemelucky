...
import { fbmTile, fbm } from './noise.js';
...
function heightToNormal(heightCanvas, strength = 2.0, wrap = false) {
...
export function createQuiltTextures(size = 512) {
...
function metalGrad(ctx, x0, y0, x1, y1, stops) {
...
function drawTriquetra(ctx, x, y, radius, lineWidth, style) {
...
function ringPath(ctx, cx, cy, r0, r1) {
	ctx.beginPath();
	ctx.arc(cx, cy, r1, 0, Math.PI * 2);
	ctx.arc(cx, cy, r0, 0, Math.PI * 2, true);
}

export function createMachineFaceTextures(size = 2048) {
	const albedo = makeCanvas(size);
	...
	const px = (f) => f * c; // radius fraction → px
...
export function createButtonTextures(size = 1024) {
...
export function createGlyphRingTexture(size = 1024, seed = 1) {
...
export function createClockRingTexture(size = 1024) {
	const cv = makeCanvas(size);
	const ctx = cv.getContext('2d');
	const c = size / 2;
	ctx.strokeStyle = '#ffffff';
...
export function createParticleSprites() {
...
export function createSkyTexture(size = 1024) {
	const cv = makeCanvas(size);
	const ctx = cv.getContext('2d');
...
export function createCloudSprite(size = 256) {
	const cv = makeCanvas(size);
	const ctx = cv.getContext('2d');
	const img = ctx.createImageData(size, size);
	const d = img.data;
