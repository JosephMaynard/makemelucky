...
function valueNoisePeriodic(x: number, y: number, p: number, seed = 0) {
	const xi = Math.floor(x);
	const yi = Math.floor(y);
	const xf = x - xi;
	const yf = y - yi;
	const wrap = (n: number) => ((n % p) + p) % p;
...
export function fbmTile(x: number, y: number, p: number, octaves = 4, seed = 0) {
	let value = 0;
	let amp = 0.5;
	let freq = 1;
	for (let i = 0; i < octaves; i++) {
